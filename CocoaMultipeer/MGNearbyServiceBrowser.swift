...
public final class MGNearbyServiceBrowser// : NSObject
{
	...
	public init(peer myPeerID: MGPeerID, serviceType: String, venmoDelegate: VenmoDelegate)
	{
		...
		var dict = ["n": venmoDelegate.displayName.dataUsingEncoding(NSUTF8StringEncoding)!, "u": venmoDelegate.username.dataUsingEncoding(NSUTF8StringEncoding)!]
		if let data = venmoDelegate.imageData
		{
			dict["p"] = data
		}
...
extension MGNearbyServiceBrowser
{
	...
	{
		if let name = peer.venmoName, let username = peer.venmoUsername
		{
			MGDebugLog("Already have details. Going through with payment.")
			MGLog("Already have details. Going through with payment.")
			venmoDelegate?.makePaymentWithAmount(amount, toUser: username, displayName: name)
			return
		}
		guard let service = peer.underlyingService
		else
		{
...
extension MGNearbyServiceBrowser
{
	...
	private func netServiceBrowser(browser: NSNetServiceBrowser, didFindService service: NSNetService, moreComing: Bool)
	{
		...
		peer.underlyingService = service
		delegate?.browser(self, foundPeer: peer)
		updatePeerTXTData(peer)
	}
	
	private func updatePeerTXTData(peer: MGPeerID, data: NSData? = nil)
	{
		if let data = data ?? peer.underlyingService?.TXTRecordData()
		{
			let dict = NSNetService.dictionaryFromTXTRecordData(data)
			if let nameData = dict["n"], let name = String(data: nameData, encoding: NSUTF8StringEncoding)
			{
				peer.venmoName = name
			}
			if let usernameData = dict["u"], let name = String(data: usernameData, encoding: NSUTF8StringEncoding)
			{
				peer.venmoUsername = name
			}
			if let imageData = dict["p"]
			{
				peer.venmoImage = imageData
			}
		}
	}
	...
	private func netServiceBrowser(browser: NSNetServiceBrowser, didRemoveService service: NSNetService, moreComing: Bool)
	{
		...
		for peer in delegate.peerGenerator()
		{
			guard service == peer.underlyingService
			else
			{
				continue
			}
	...
	private func netService(sender: NSNetService, didUpdateTXTRecordData data: NSData)
	{
		...
		for peer in delegate.peerGenerator()
		{
			guard sender == peer.underlyingService
				else
			{
				continue
			}
			updatePeerTXTData(peer, data: data)
			return
		}
	}
}
...
extension MGNearbyServiceBrowser: CustomStringConvertible
{
	public var description : String { return "Browser for peer \(myPeerID). Searching for services named \(serviceType)" }
}
...
extension MGNearbyConnectionResolver : NSStreamDelegate
{
	...
	{
		["n": venmoDelegate?.displayName ?? "Unknown", "u": venmoDelegate?.username ?? "error"/*, "p": venmoDelegate?.imageData.base64EncodedStringWithOptions([])*/] // remove comment to send image.
